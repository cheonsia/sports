		...
		<script type="text/javascript">
			...
			function peopleChoose(ths){
				...
				if($(ths).is(':checked')){//체크했을 때
   					...
   					var people_div = "";
   					people_div += "<div id='player_num" + ths_num + "' data-id='player_num" + ths_num + "' class='player_box' ondragstart='playerDragStart(this, event)' ondragend='playerDragEnd(this, event)' data-number='" + ths_num + "'>";
   					people_div += "	<img src='./image/baseball/player/" + ths_img + "' alt='" + ths_val + " 이미지'>";
   					people_div += "</div>";
			...
			function strategyDeleteOk(){
				...
				var area_kor = $('input[name="area_kor"]').val();
				var area_eng = $('input[name="area_eng"]').val();
				location.href="baseballStrategyDelete?name=" + area_eng + "&area=" + area_kor + "&stnum="+strategy_stnum;
			}
			/*전략 수정*/
		...
		<div class="strategy_whole">
			<div class="strategy_inner">
				...
				<div class="strategy_box">
					<c:if test="${strategyPlayerList != null}">
						<!-- 불러오기 및 수정 영역 -->
						<form action="baseballStrategyUpdate" method="post" id="strategy_form_update">
							...
							<script type="text/javascript">
								$(function(){
									...
									for(var i = 0; i < (stinfoArray.length / 5); i++){
										var people_div = "";
											people_div += "<div id='player_num" + stinfoArray[(5 * i) + 1] + "' data-id='player_num" + stinfoArray[(5 * i) + 1] + "' class='player_box' ondragstart='playerDragStart(this, event)' ondragend='playerDragEnd(this, event)' data-type='change' data-number='" + stinfoArray[(5 * i) + 1] + "'>";
											people_div += "	<img src='./image/baseball/player/" + stinfoArray[(5 * i) + 2] + "' alt='" + stinfoArray[5 * i] + " 이미지'>";
											people_div += "</div>";
											$('.strategy_map').append(people_div);
										if(win_wid >= 1250){
							...
							<img alt="경기장 이미지" src="./image/baseball/icon/baseball_play_bg.png" class="strategy_bg">
					...
					<c:if test="${empty strategyPlayerList}">
	  					<!-- 처음 시작 영역 -->
						<form action="baseballStrategySave" method="post" id="strategy_form">
							<c:if test="${areaset != null}">
								<input type="hidden" value="${area.area_eng}" name="area_eng">
								<input type="hidden" value="${area.area_kor}" name="area_kor">
							</c:if>
