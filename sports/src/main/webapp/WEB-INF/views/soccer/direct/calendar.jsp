	...
	<body>
				...
				<div class="traning_show_box">
					...
					<div class="traning_show_inner">
						<form action="soccerCalendarUpdate" id="soccer_calendar_update" method="post">
							...
							<input type="hidden" id="calendar_info_val2" name="calendar_info_val2" value="soccer">
		...
		<script type="text/javascript">
			function calendarMove(ths){//이전/다음달 버튼 클릭 시
				...
				location.href='soccerCalendarMove?name='+area_eng+'&area='+area_kor+'&start='+wholestart;
			}
			//스케줄
			...
			function listGet(ths){
				...
				var area_kor = $('#area_kor').val();
				wholestart = startyy + '-' + startmm + '-01';
				location.href='soccerCalendarGet?name='+area_eng+'&area='+area_kor+'&start='+wholestart+'&number='+ths_type;
				listShow();
			}
			function listShow(){
				$('body').css('overflow', 'hidden');
			...
			function listDelete() {
				...
				window.location.href='soccerCalendarDelete?name='+area_eng+'&area='+area_kor+'&start='+startdate+'&num='+num;
			}
		</script>
	</body>
</html>